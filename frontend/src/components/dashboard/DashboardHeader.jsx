...
import { Search, Bell, MessageSquare, Sun, Moon, ChevronDown } from 'lucide-react';
                ...
                <div className="flex items-center gap-3">
                    ...
                    >
                        <div className="relative h-5 w-10 rounded-full bg-[#E2E8F0] dark:bg-secondary-700 flex items-center px-1">
                            <div className={`h-4 w-4 rounded-full bg-white shadow-sm transition-transform ${theme === 'dark' ? 'translate-x-4' : 'translate-x-0'}`} />
                        </div>
                    </button>
                ...
                <Menu as="div" className="relative">
                    <MenuButton className="flex items-center gap-3 focus:outline-none">
                        <div className="text-right hidden sm:block">
                            ...
                            <p className="text-xs text-secondary-500">{user?.role === 'organizer' ? 'Event Organizer' : 'Attendee'}</p>
                        </div>
                    ...
                    <MenuItems className="absolute right-0 mt-3 w-48 origin-top-right rounded-md bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none dark:bg-secondary-800 dark:ring-white/10">
                        <MenuItem>
                            <a href="/profile" className="block px-4 py-2 text-sm text-secondary-700 dark:text-secondary-200 hover:bg-secondary-100 dark:hover:bg-secondary-700">My Profile</a>
                        </MenuItem>
                        <MenuItem>
                        ...
                        <MenuItem>
                            <button onClick={handleLogout} className="block w-full text-left px-4 py-2 text-sm text-red-600 hover:bg-secondary-100 dark:hover:bg-secondary-700">Logout</button>
                        </MenuItem>
                    </MenuItems>
                </Menu>
            </div>
