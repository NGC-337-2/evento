...
const EventCard = ({ event }) => {
  ...
  return (
    <Link to={`/event/${event._id}`} className="group relative flex flex-col bg-white dark:bg-secondary-800 rounded-2xl ring-1 ring-secondary-200 dark:ring-secondary-700 sm:p-5 transition-all duration-300 hover:shadow-lg hover:-translate-y-0.5 overflow-hidden sm:overflow-visible">
      <div className="relative aspect-[3/2] sm:aspect-[2/1] lg:aspect-[3/2] overflow-hidden sm:rounded-xl">
        ...
        <div className="absolute top-3 left-3 bg-white/95 dark:bg-secondary-900/95 backdrop-blur-sm px-2.5 py-1.5 rounded-lg shadow-sm border border-secondary-200/50 dark:border-secondary-700/50">
          <div className="flex flex-col items-center justify-center text-primary-600 dark:text-primary-400">
            <span className="text-xs font-bold uppercase tracking-widest">{format(startDate, 'MMM')}</span>
            <span className="text-lg font-black leading-none mt-0.5">{format(startDate, 'dd')}</span>
          </div>
        </div>
      ...
      <div className="flex flex-1 flex-col p-4 sm:p-0 sm:pt-4">
        ...
        <div className="mt-3 flex flex-col gap-2 text-sm text-secondary-600 dark:text-secondary-400">
          <div className="flex items-center gap-2">
            <CalendarIcon className="h-4 w-4 shrink-0 text-secondary-400 dark:text-secondary-500" aria-hidden="true" />
            <span className="truncate">{format(startDate, 'EEEE, h:mm a')}</span>
          </div>
          <div className="flex items-start gap-2">
            <MapPin className="h-4 w-4 shrink-0 mt-0.5 text-secondary-400 dark:text-secondary-500" aria-hidden="true" />
            <span className="line-clamp-1">{event.location?.city || event.location?.address}</span>
          </div>
        </div>
        ...
        <div className="mt-4 pt-4 border-t border-secondary-100 dark:border-secondary-700 flex items-center justify-between mt-auto relative z-10">
          <div className="flex flex-col">
            <span className="text-[10px] text-secondary-500 dark:text-secondary-400 uppercase tracking-widest font-semibold mb-0.5">Tickets from</span>
