...
exports.createBooking = async (req, res) => {
  ...
  for (const requestedTicket of tickets) {
    ...
    if (tier.capacity - tier.sold < requestedTicket.quantity) {
      const err = new Error(
        `Not enough tickets available for tier: ${tier.name}`
      );
      err.statusCode = 400;
      throw err;
    }
  ...
  const booking = await Booking.create({
    ...
    totalPrice,
    status:
      totalPrice === 0 ? BOOKING_STATUS.CONFIRMED : BOOKING_STATUS.PENDING,
...
exports.cancelBooking = async (req, res) => {
  ...
  if (req.user.role === 'admin') {
    await logAudit(req, 'CANCEL_BOOKING', 'Booking', booking._id, {
