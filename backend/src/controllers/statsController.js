...
exports.getDashboardStats = async (req, res) => {
  try {
    ...
    const [totalUsers, totalEvents, totalBookings, revenueAgg] =
      await Promise.all([
        User.countDocuments({}),
...
exports.getEventStats = async (req, res) => {
  ...
  const bookings = await Booking.find({
    ...
    .select('quantity totalPrice status createdAt')
    .sort({ createdAt: -1 });
  ...
  const conversionRate =
    event.capacity > 0 ? ((capacityUsed / event.capacity) * 100).toFixed(1) : 0;
  ...
  try {
    ...
    const eventPerformance = events.map((e) => {
      const eventBookings = bookings.filter(
        (b) => b.event.toString() === e._id.toString()
      );
      return {
        _id: e._id,
        title: e.title,
